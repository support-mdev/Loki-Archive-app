    ...
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}">
        ...
        <div class="flex gap-2 items-center justify-between sm:hidden">
            ...
            @if ($paginator->onFirstPage())
                <span class="inline-flex items-center px-4 py-2 text-sm font-medium text-green-600 bg-white border border-green-300 cursor-not-allowed leading-5 rounded-md dark:text-gray-300 dark:bg-gray-700 dark:border-gray-600">
                    {!! __('pagination.previous') !!}
                </span>
            @else
                <a href="{{ $paginator->previousPageUrl() }}" rel="prev" class="inline-flex items-center px-4 py-2 text-sm font-medium text-green-600 bg-white border border-green-300 leading-5 rounded-md hover:text-green-700 focus:outline-none focus:ring ring-green-300 focus:border-green-300 active:bg-green-100 active:text-green-800 transition ease-in-out duration-150 dark:bg-green-800 dark:border-green-600 dark:text-green-200 dark:focus:border-green-700 dark:active:bg-green-700 dark:active:text-green-300 hover:bg-green-100 dark:hover:bg-green-900 dark:hover:text-green-200">
                    {!! __('pagination.previous') !!}
                </a>
            @endif

            @if ($paginator->hasMorePages())
                <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center px-4 py-2 text-sm font-medium text-green-600 bg-white border border-gray-300 leading-5 rounded-md hover:text-green-700 focus:outline-none focus:ring ring-green-300 focus:border-green-300 active:bg-green-100 active:text-green-800 transition ease-in-out duration-150 dark:bg-green-800 dark:border-green-600 dark:text-green-200 dark:focus:border-green-700 dark:active:bg-green-700 dark:active:text-green-300 hover:bg-green-100 dark:hover:bg-green-900 dark:hover:text-green-200">
                    {!! __('pagination.next') !!}
                </a>
            @else
                <span class="inline-flex items-center px-4 py-2 text-sm font-medium text-green-600 bg-white border border-green-300 cursor-not-allowed leading-5 rounded-md dark:text-gray-300 dark:bg-gray-700 dark:border-gray-600">
        ...
        <div class="hidden sm:flex-1 sm:flex sm:gap-2 sm:items-center sm:justify-between">
            ...
            <div>
                <p class="text-sm text-green-700 leading-5 dark:text-green-600">
                    {!! __('Showing') !!}
